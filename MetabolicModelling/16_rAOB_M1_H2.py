...
def calc_dGcat_Ex(logC_substrates):
	# 4 CO2 (g) + 13 H2 (g) → C4H10 (g) + 8 H2O  dG0=-337.2
	#dG=-199.9
...
def print_log10C(current_logC,all_substrates):
	report=['C4H10 (g)','C4H9-S-CoM','Butyryl-CoA','Crotonyl-CoA','(S)-3-Hydroxybutyryl-CoA','Acetoacetyl-CoA','Acetyl-CoA','CH3-H4MPT','CH2=H4MPT','CH≡H4MPT+','HCO-H4MPT','HCO-MF','CO2 (g)','F420H2','NADH','Fdred','MPH2','XH2']#,'Acetyl-CoA','CO2 (g)']
	for s in report:
		indice=all_substrates.index(s)
		value=current_logC[indice]
		print('log10C\t%s\t%.3f'%(s,math.log10(math.exp(value))))
...
# 		fout1.write('%.1e\t%s\t%s\t%.3f\n'%(conc_XH2,i,s,math.log10(math.exp(value))))

# 	for i in dG_set:
# 		index=i['reaction index']
# 		fout2.write('%.1e\t%.2f\t%s\t%.2f\t%s\n'%(conc_XH2,dG_overall,i['name'],i['dG'],reactions[index]['sto']))

# 	fout2.write('%.1e\t%.2f\t%s\t%.2f\t%s\n'%(conc_XH2,dG_overall,name_of_reaction_for_global_dG_calc,calc_global_dG(current_logC),reactions[reaction_for_global_dG_calculation]['sto']))
...
dG_CoA2CoM=[-40,-60,-80,-100,-120,-140]
# NADH oxidation: 7 (index); 9 (line number in the table)
dG_NAD_red=[0,-10]
# F420H2 oxidation: 14 (index); 16 (line number in the table)
dG_F420_red=[0,10,20,30]
# MPH2 oxidation: 16 (index): 18 (line number in the table)
#dG_MP_red=[0,-10,-20,-30]
# Fdred oxidation: 15 (index); 17 (line number in the table)
dG_Fd_red=[20,40,60,80,100,120]
# methylene-H4MPT oxidation: 10 (index); 12 (line number in the table)
dG_CH_red=[0,-10]
# acetyl-coa synthesis: 8 (index); 10 (line number in the table)
dG_acetyl_coa_syn=[0,-10,-30,-50]


dG=[{'name':'CoA2CoM (build-up of proton gradient)','dG_range':dG_CoA2CoM,'reaction index':2},
	{'name':'NAD_red (energy dissipation)','dG_range':dG_NAD_red,'reaction index':7},
	{'name':'F420_red (consumption of proton gradient)','dG_range':dG_F420_red,'reaction index':14},
	#{'name':'MP_red','dG_range':dG_MPH2_ox,'reaction index':16},
	{'name':'Fd_red (consumption of proton gradient)','dG_range':dG_Fd_red,'reaction index':15},
	{'name':'CH_red (energy dissipation)','dG_range':dG_CH_red,'reaction index':10},
	{'name':'acetyl_coa_syn (energy dissipation)','dG_range':dG_acetyl_coa_syn,'reaction index':8}]
...
model = 'rAOB_M1'
infile='16_rAOB_M1.txt'
EX = 'Ex = -0.414V'
dG0_column = 5
conc_XH2_list=[5e-3,1e-3,5e-4,1e-4,5e-5]
name_of_reaction_for_global_dG_calc='MQ_red (build-up of proton gradient)'
reaction_for_global_dG_calculation=16
output_prefix='16_rAOB_M1_H2__res'
...
reactions,dG0_overall=get_all_reactions(infile,dG0_column)
all_substrates=get_all_substrates(reactions)


for conc_XH2 in conc_XH2_list:#[50,100,500,1000]:
	logC_substrates={'C4H10 (g)':math.log(0.1),'CO2 (g)':math.log(0.3),'XH2':math.log(conc_XH2)}
